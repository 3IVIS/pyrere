...
def _module_node_id(graph: CodeGraph, abs_path: str) -> Optional[str]:
    """Return the module node ID for `abs_path`, or None."""
    for node in graph.nodes.values():
        if (node.type == "module"
                and node.file
                and os.path.abspath(node.file) == abs_path):
            return node.id
    return None
...
def run_grimp(repo_root: str, graph: CodeGraph) -> int:
    ...
    # Ensure the repo root is importable so grimp can resolve the packages
        ...
        for importer_mod in ig.modules:
            ...
            for imported_mod in imported_mods:
                ...
                if edge_id in graph.edges:
                    # Confirmed by grimp — nudge confidence up and record source
                    existing = graph.edges[edge_id]
                    existing.confidence = min(1.0, existing.confidence + 0.04)

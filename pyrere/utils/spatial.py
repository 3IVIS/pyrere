...
def build_spatial_index(graph: CodeGraph) -> dict[str, list[tuple[int, int, str]]]:
    ...
    for node in graph.nodes.values():
        ...
        index.setdefault(path, []).append(
            (node.span[0], node.span[1], node.id)
        )
...
def module_node_for(graph: CodeGraph, abs_path: str) -> Optional[str]:
...
def stamp_issue(graph: CodeGraph, node_id: Optional[str], issue: dict) -> None:
